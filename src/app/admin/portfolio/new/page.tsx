...
export default function NewPortfolioPage() {
  ...
  // Multi Gallery Image Upload Handler
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setErrorMsg(data.error || "Failed to save portfolio");
      }
    } catch (err) {
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
        <h1 className="text-2xl font-serif font-bold text-slate-900 flex items-center gap-2">
          <FolderKanban className="w-6 h-6 text-amber-500" /> Create New Portfolio Item
        </h1>
        <p className="text-slate-500 text-xs mt-1">
          Upload project cover image, multiple gallery photos, assign category, tags, and SEO settings.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-8">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Royal Red Mandap & Wedding Feast"
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Brief summary shown on cards and sliders..."
              className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3 text-sm focus:ring-2 focus:ring-amber-500"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Full details about catering menu, decoration elements, venue setup..."
              className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3 text-sm focus:ring-2 focus:ring-amber-500"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Wedding, Royal Mandap, Live Counters, Delhi NCR"
              className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3 text-sm focus:ring-2 focus:ring-amber-500"
            />
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
              {coverImage ? (
                ...
                <label className="w-full sm:w-64 h-24 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50 hover:bg-slate-100/80 flex flex-col items-center justify-center cursor-pointer transition-colors">
                  {uploadingCover ? (
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-2">
              Unlimited Photo Gallery ({galleryImages.length} images added)
            </label>
              ...
              <label className="h-28 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50 hover:bg-slate-100 flex flex-col items-center justify-center cursor-pointer transition-colors">
                {uploadingGallery ? (
                  ...
                  <>
                    ...
                    <span className="text-xs font-bold text-amber-600">Add Gallery Photos</span>
                  </>
                )}
                <input
                  type="file"
        ...
        <div className="pt-4 flex justify-end border-t border-slate-100">
          ...
          >
            {submitting ? <Loader2 className="w-5 h-5 animate-spin" /> : <Save className="w-5 h-5" />} Save Portfolio
          </button>
        </div>
      </form>
    </div>
