  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {/* Total Leads */}
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex items-center gap-4 relative overflow-hidden group hover:shadow-md transition-shadow">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-900 mt-0.5">{totalLeads}</h3>
          </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex items-center gap-4 relative overflow-hidden group hover:shadow-md transition-shadow">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-900 mt-0.5">
              {newLeads} <span className="text-xs font-normal text-slate-400">new / {contactedLeads} contacted</span>
            </h3>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-6">
          <div>
            ...
            <p className="text-slate-500 text-xs mt-0.5">Breakdown of recent inquiries by event type</p>
          </div>

          <div className="space-y-4">
            {Object.keys(eventTypeCounts).length === 0 ? (
              <p className="text-slate-400 text-xs py-4 text-center">No recent data to plot.</p>
            ) : (
        ...
        <div className="lg:col-span-5 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-6">
          <div>
            ...
            <p className="text-slate-500 text-xs mt-0.5">Active conversion funnel breakdown</p>
          </div>
            ...
            {[
              ...
              return (
                <div key={stage.label} className="flex items-center justify-between gap-4 text-xs">
                  <span className="w-24 font-bold text-slate-600">{stage.label}</span>
                  <div className="flex-1 bg-slate-100 h-4 rounded-md overflow-hidden relative">
                    <div
                      className={`${stage.color} h-full transition-all duration-500`}
                      style={{ width: `${barWidth}%` }}
                    />
                  </div>
                  <span className="w-8 text-right font-bold text-slate-900">{stage.count}</span>
                </div>
              );
            })}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col justify-between">
          <div>
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              <div>
                <h3 className="font-serif font-bold text-lg text-slate-900">Recent Customer Enquiries</h3>
                <p className="text-slate-500 text-xs mt-0.5">Latest form submissions from website</p>
              </div>
              <Link
                href="/admin/leads"
        ...
        {/* Audit Logs Timeline */}
        <div className="lg:col-span-5 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-6 flex flex-col justify-between">
          <div>
            <h3 className="font-serif font-bold text-lg text-slate-900 flex items-center gap-2">
              <Activity className="w-5 h-5 text-amber-500" /> Recent Actions Log
            </h3>
            <p className="text-slate-500 text-xs mt-0.5">Timeline of recent site modification actions</p>
          </div>
          ...
          <div className="space-y-4 flex-1 mt-4">
            {recentLogs.length === 0 ? (
              ...
              recentLogs.map((log) => (
                <div key={log.id} className="flex items-start gap-3 border-l-2 border-slate-200 pl-4 relative">
                  <span className="w-2.5 h-2.5 rounded-full bg-amber-500 absolute -left-[6px] top-1" />
                  <div className="text-xs">
                    <p className="font-bold text-slate-800">
                      {log.user.name} <span className="font-normal text-slate-500">({log.action})</span>
                    </p>
                    <p className="text-[10px] text-slate-600 font-mono mt-0.5">{log.details}</p>
